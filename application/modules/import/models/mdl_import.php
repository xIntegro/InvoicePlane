...
{
    public $table = 'xc_imports';
    public $primary_key = 'xc_imports.import_id';
    ...
    public $primary_keys = array(
        ...
        'xc_invoices' => 'invoice_id',
        'xc_invoice_items' => 'item_id',
        'xc_payments' => 'payment_id'
    );
    ...
    {
        $this->db->select("SQL_CALC_FOUND_ROWS xc_imports.*,
            (SELECT COUNT(*) FROM xc_import_details WHERE import_table_name = 'xc_clients' AND xc_import_details.import_id = xc_imports.import_id) AS num_clients,
            (SELECT COUNT(*) FROM xc_import_details WHERE import_table_name = 'xc_invoices' AND xc_import_details.import_id = xc_imports.import_id) AS num_invoices,
            (SELECT COUNT(*) FROM xc_import_details WHERE import_table_name = 'xc_invoice_items' AND xc_import_details.import_id = xc_imports.import_id) AS num_invoice_items,
            (SELECT COUNT(*) FROM xc_import_details WHERE import_table_name = 'xc_payments' AND xc_import_details.import_id = xc_imports.import_id) AS num_payments", FALSE);
    }

    public function default_order_by()
    {
        $this->db->order_by('xc_imports.import_date DESC');
    }
    ...
    public function start_import()
    {
        ...
        $this->db->insert('xc_imports', $db_array);
                ...
                foreach ($headers as $key => $header) {
                    if ($header == 'user_email') {
                        ...
                        if ($client->num_rows()) {
                            ...
                            $this->db->insert('xc_clients', $client_db_array);
                ...
                foreach ($headers as $key => $header) {
                    if ($header == 'invoice_number') {
                        ...
                        if ($data[$key] > 0) {
                            ...
                            $tax_rate = $this->db->get('xc_tax_rates');
                            if ($tax_rate->num_rows()) {
                                $data[$key] = $tax_rate->row()->tax_rate_id;
                            } else {
                                $this->db->insert('xc_tax_rates', array('tax_rate_name' => $data[$key], 'tax_rate_percent' => $data[$key]));
                                $data[$key] = $this->db->insert_id();
                            }
                        } else {
                            $data[$key] = 0;
                ...
                foreach ($headers as $key => $header) {
                    if ($header == 'invoice_number') {
                        ...
                        if ($data[$key]) {
                            ...
                            $payment_method = $this->db->get('xc_payment_methods');
                            if ($payment_method->num_rows()) {
                                $data[$key] = $payment_method->row()->payment_method_id;
                            } else {
                                $this->db->insert('xc_payment_methods', array('payment_method_name' => $data[$key]));
                                $data[$key] = $this->db->insert_id();
                            }
                        } else {
    ...
    public function record_import_details($import_id, $table_name, $import_lang_key, $ids)
    {
        foreach ($ids as $id) {
            ...
            $this->db->insert('xc_import_details', $db_array);
        }
    }
    ...
    public function delete($import_id)
    {
        ...
        $import_details = $this->db->where('import_id', $import_id)->get('xc_import_details')->result();
        ...
        $this->db->delete('xc_import_details');
